...
searchButton.addEventListener("click", (event) => {
    const userInput = document.getElementById("search").value.toUpperCase();
    console.log(userInput);

    const results = hikes.filter((hike) => {
        if (hike.name.toUpperCase().includes(userInput)) 
            return true
        if (hike.description.toUpperCase().includes(userInput)) 
            return true
        for (const tag of hike.tags)
            if (tag.toUpperCase().includes(userInput)) 
                return true
        return false
    });
    results.sort((a,b) => {
    ...
    for (const hike of results){
        html += generateHikeCard(hike);
    }

    document.getElementById("hike-container").innerHTML = html;
})

function generateHikeCard(hike){
