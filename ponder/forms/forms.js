...
function updateNotesField() {
    ...
    notesContainer.hidden = value === "many" ? false : true;
}
...
form.addEventListener("submit", function (event) {
    ...
    if (type === "one" && selectedCampuses.length == 0){
        output.textContent = "You need to select at least one campus."
        return;
    }
    ...
    if (type === "many" && note.length == 0) {
        output.textContent = "You have a skill issue, please try again."
        return;
    }
    ...
    if (type === "many" && selectedCampuses.length < 2){
        output.textContent = "First name has already been taken, please try again."
        return;
    }
