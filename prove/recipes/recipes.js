...
const recipeGallery = document.getElementById("recipe_gallery");

recipeGallery.innerHTML = generateRecipeCard(recipes[Math.floor(Math.random() * recipes.length)]);
...
searchForm.addEventListener("submit", function (event) {
    ...
    const recipeResults = filterRecipes(query).sort((recipeA,recipeB) => {
    ...
    for (let recipe of recipeResults){
        galleryHTML += generateRecipeCard(recipe);
    }

    recipeGallery.innerHTML = galleryHTML;
});
...
function filterRecipes(query){
    return recipes.filter((recipe) => {
        ...
        // if (recipe.author.toUpperCase().includes(queryUpper)) 
        //     return true;
        if (recipe.name.toUpperCase().includes(queryUpper)) 
            return true;
        if (recipe.description.toUpperCase().includes(queryUpper)) 
            return true;
        for (let tag of recipe.tags)
            if (tag.toUpperCase().includes(queryUpper))
                return true;
        for (let ingredient of recipe.recipeIngredient)
            if (ingredient.toUpperCase().includes(queryUpper)) 
                return true
...
function generateRecipeCard(recipe){
    let cardHTML = `
        <div class="recipe_card">
            <img src="${recipe.image}" alt="Finished Recipe Preview">
            <div class="recipe_content">
                <div class="recipe_tag_list">
    `;

    for (let tag of recipe.tags)
        cardHTML += `
                    <button class="recipe_tag">${tag}</button>
        `;
    ...
    for (let i = 0; i < (Math.floor(5 - recipe.rating)); i++)
        cardHTML += `
